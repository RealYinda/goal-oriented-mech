...
*************************************************************************
*
* @brief 基于JAUMIN框架的在非结构网格上, 求解Poisson方程.
*
* 该程序分以下几个步骤:
...
int main(int argc, char* argv[]) {
  ...
  tbox::JAUMINManager::startup();
  {
    ...
    // 创建网格片算法策略类.
    tbox::Pointer<PatchStrategy> patch_strategy = NULL;
    if (fem_db->getInteger("element_order") == 2)
      patch_strategy = new TwoOrderPatchStrategy("Poisson_2_PatchIntegrator",
                                                 is_from_restart, fem_db);
    else if (fem_db->getInteger("element_order") == 1)
      patch_strategy = new OneOrderPatchStrategy("Poisson_1_PatchIntegrator",
                                                 is_from_restart, fem_db);

    /// 创建网格层时间积分算法类（应用级: 提供有限元求解流程定制支撑）.
    tbox::Pointer<PoissonLevelStrategy> poisson_level_integrator =
        new PoissonLevelStrategy("PoissonLevelIntegrator", patch_strategy,
    ...
    tbox::Pointer<algs::HierarchyTimeIntegrator<NDIM> > time_integrator =
        new algs::HierarchyTimeIntegrator<NDIM>(
            ...
            poisson_level_integrator, is_from_restart);
    ...
    if (javis_dump_interval > 0) {
      javis_data_writer = new appu::JaVisDataWriter<NDIM>(
          "Poisson_JaVis_Writer", javis_dump_dirname,
          javis_number_procs_per_file);

      patch_strategy->registerPlotData(
    ...
    tbox::pout << "\n\n++++++++++++++++++++++++++++++++++++++++++++" << endl;
    tbox::pout << "At end of solving # " << dt_actual << endl;
    tbox::pout << "++++++++++++++++++++++++++++++++++++++++++++\n\n" << endl;

    // 输出可视化数据.
    if (javis_dump_interval > 0) {
      javis_data_writer->writePlotData(time_integrator->getPatchHierarchy(), 1,
                                       1.0);
    }
    /************************************************************************************
     *                               模  拟  结  束 *
